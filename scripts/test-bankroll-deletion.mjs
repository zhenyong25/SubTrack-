...
const loadService = async (remote) => {
  const result = await build({
    stdin: { contents: "export { deletePokerMttBankrollTransaction } from './services/appDataService';", resolveDir: process.cwd(), loader: 'ts' },
...
globalThis.__investmentClient = {
  ...
  from(table) {
    assert.equal(table, 'poker_mtt_bankroll_transactions');
    filters = {};
    return {
      delete() { return this; },
...
seed();
await remote.deletePokerMttBankrollTransaction('first');
assert.deepEqual(filters, { user_id: 'test-user', id: 'first' });
assert.deepEqual(read(), [{ id: 'second' }]);
await remote.deletePokerMttBankrollTransaction('second');
...
for (const scenario of ['failure', 'missing', 'signed-out']) {
  ...
  await assert.rejects(() => remote.deletePokerMttBankrollTransaction('first'));
  assert.deepEqual(read(), [{ id: 'first' }, { id: 'second' }], `${scenario} must preserve local entries`);
}
console.log('Bankroll deletion checks passed: local, remote, final entry, scoped deletion, and failure preservation.');

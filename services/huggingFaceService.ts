...
const buildTransactionPrompt = (chunkText: string, currencyHint?: string) => `You are reading raw text extracted from a bank or credit card e-statement. Extract every individual
...
- transactionDate: ISO date (YYYY-MM-DD). Infer the year from the statement period if a line only shows day/month.
...
export const parseStatementTransactions = async (
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    const response = await fetch(HF_CHAT_ENDPOINT, {
      ...
      body: JSON.stringify({
        model: HF_MODEL,
        messages: [{ role: 'user', content: buildTransactionPrompt(chunks[i], currencyHint) }],
        temperature: 0,
        max_tokens: 8000,
      }),
    });
